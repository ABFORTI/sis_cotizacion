    ...
    <form action="{{ route('costeo.store', $cotizacion->id) }}" method="POST" id="costeoForm">
        ...
        <fieldset>
            ...
            <div class="mb-6">
                ...
                <thead class="bg-[#848484] text-white">
                    ...
                    <tbody>
                        <tr>
                            <td class="border border-gray-300 bg-gray-50 p-2 font-bold">Molde</td>
                            <td class="border border-gray-300 bg-gray-50 p-2">
                                <input name="molde_ancho"
                                    id="input-molde-ancho"
                                    value="{{ $molde_ancho }}"
                                    class="w-full p-2 text-center font-bold" readonly>
                            </td>
                            <td class="border border-gray-300 bg-gray-50 p-2">
                                <input name="molde_avance"
                                    id="input-molde-avance"
                                    value="{{ $molde_avance }}"
                                    class="w-full p-2 text-center font-bold" readonly>
                            </td>
                        </tr>
        ...
        <script>
            ...
            function calcularAnchoHoja() {
                ...
                if (suma40.includes(maquina)) {
                    incremento = 40;
                } else if (suma50.includes(maquina)) {
                    incremento = 50;
                } else if (suma100.includes(maquina)) {
                    incremento = 100;
                }

                const resultado = moldeAncho + incremento;

                document.getElementById('input-hoja-ancho').value = resultado;

                calcularAreaFormadoHoja();
                calcularParedMedia();
            }
                
            function calcularAvanceHoja() {
                ...
                const resultado = moldeAvance + incremento;

                document.getElementById('input-hoja-avance').value = resultado;
        ...
        <fieldset>
            ...
            <script>
                ...
                function calcularCostoTermoformado() {
                    ...
                    const costoPorPersonaPorDia = 1102; // Costo fijo por persona por día
                    const totalHojasPorTurno = parseFloat(document.querySelector('input[name="total_hojas_turno_termoformado"]').value) || 1;
        ...
        <script>
            function calcularPiezasPorCaja() {
                ...
                const mult = parseInt(document.querySelector('input[name="aux_piezas_por_bolsa"]').value) || 1;
                ...
                <thead class="bg-[#848484] text-white">
                    <tr class="border-b font-semibold">
                        ...
                        <th class="px-2 py-2">
                            ...
                            <script>
                                function toggleHrsMaquinadoMolde() {
                                    const inputCostoHora = document.getElementById('input-costo-hora-maquinado');
                                    const gridHeader = document.getElementById('grid-header-hrs-maquinado');
                ...
                <tbody>
                    <tr class="border-b h-12">
                        <td class="text-left px-2 font-medium">
                            <div class="grid gap-2 items-center" id="grid-molde">
                                <button type="button" onclick="toggleCostoAluminio()" class="font-medium px-3 py-1 rounded bg-blue-500 text-white hover:bg-blue-600 transition-colors duration-200 text-center" title="Ocultar/Mostrar el costo de aluminio para calcular el costo del material del molde">
                                    MOLDE
                                </button>
                    ...
                    <tr class="border-b h-12">
                        <td class="text-left px-2 font-medium">
                            <div class="grid gap-2 items-center" id="grid-empujador">
                                <button type="button" onclick="toggleCostoEmpujador()" class="font-medium px-3 py-1 rounded bg-blue-500 text-white hover:bg-blue-600 transition-colors duration-200 text-center" title="Ocultar/Mostrar el costo auxiliar para calcular el costo del material del empujador">
                                    EMPUJADOR
                                </button>
                    ...
                    <script>
                        ...
                        function calcularTotalEmpujador() {
                            ...
                            const costoHoraMaquinado = parseFloat(document.querySelector('input[name="costo_hora_maquinado"]').value) || 60; // Valor por defecto $60
                            const resultado = costoEmpujador + (hrsMaquinadaEmpujador * costoHoraMaquinado);
                        ...
                        function calcularTotalMolde() {
                            ...
                            const costoHoraMaquinado = parseFloat(document.querySelector('input[name="costo_hora_maquinado"]').value) || 60; // Valor por defecto $60
                            const resultado = costoMolde + (hrsMaquinadaMolde * costoHoraMaquinado);
                    ...
                    <tr class="border-b h-12">
                        <td class="text-left px-2 font-medium">
                            <div class="grid gap-2 items-center" id="grid-muestras">
                                <button type="button" onclick="toggleCostoMuestras()" class="font-medium px-3 py-1 rounded bg-blue-500 text-white hover:bg-blue-600 transition-colors duration-200 text-center" title="Ocultar/Mostrar los campos para calcular el costo de muestras (Número de muestras × Costo por muestra)">
                                    MUESTRAS
                                </button>
                    ...
                    <tr class="border-2 border-gray-500 bg-gray-50 font-semibold">
                        ...
                        <td class="text-center p-2">
                            <input step="0.01" name="TOTAL_FINAL" value="{{old('TOTAL_FINAL', $costeoRequisicion->TOTAL_FINAL)}}"
                                class="w-full p-2 text-center font-bold" placeholder="Total general" oninput="calcularTotalVentas()" readonly>
                        </td>
                    </tr>
                    <tr class="border-2 border-gray-500 bg-gray-50 font-semibold">
                        <td colspan="3" class="text-right text-gray-700 px-2" onclick="calcularTotalVentas()">TOTAL VENTAS</td>
                        <td class="text-center p-2">
                            <input step="0.01" name="TOTAL_VENTAS" value="{{old('TOTAL_VENTAS', $costeoRequisicion->TOTAL_VENTAS)}}"
                                class="w-full p-2 text-center font-bold" placeholder="Total ventas MXN" readonly>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <table class="w-full text-center border-collapse border border-gray-400">
                ...
                <tfoot>
                    ...
                    <tr class="bg-gray-50 font-bold text-center">
                        ...
                        <td class="p-2">
                            <input step="0.0001" name="resumen_total_costo_adicionales" readonly
                                value="{{ old('resumen_total_costo_adicionales', $costeoRequisicion->resumen_total_costo_adicionales ?? '') }}"
                                class="w-full p-2 text-center font-bold">
                        </td>
                        <td class="p-2">
                            <input name="resumen_total_piezas_adicionales" readonly value="1"
                                class="w-full p-2 text-center font-bold">
                        </td>
                        <td class="p-2">
                            <input step="0.0001" name="resumen_total_costo_unit_adicionales" readonly
        ...
        @if($esCorridaPiloto)
        <div class="mt-4 p-4 bg-gray-50 border-4 border-gray-300">
            <h3 class="text-lg font-semibold text-gray-800">Comentarios de costeo</h3>
            <p class="text-gray-700">Los costos de la Fabricación de CP no se reflejan en el resumen de costos final. Se puede encontrar en la tabla inferior "COSTOS DE CORRIDA PILOTO".</p>
        </div>
        <br>
        @endif
        ...
        <fieldset>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    <label for="tiempo_herramientas" class="block text-sm font-medium text-gray-700 mb-2">Tiempo elaboración de herramientales</label>
                    <div class="flex gap-3 items-center mt-1">
                        <input type="number" id="tiempo_herramientas_min" class="w-24 rounded-md border-gray-300 shadow-sm p-2 text-center" placeholder="15" oninput="concatenarTiempoHerramientales()">
                        <span class="text-gray-600 font-medium">a</span>
                        <input type="number" id="tiempo_herramientas_max" class="w-24 rounded-md border-gray-300 shadow-sm p-2 text-center" placeholder="20" oninput="concatenarTiempoHerramientales()">
                        <span class="text-gray-600 font-medium">días hábiles</span>
                    </div>
                    <input type="hidden" name="tiempo_herramientas" id="tiempo_herramientas" value="{{ old('tiempo_herramientas', $costeoRequisicion->tiempo_herramientas) }}">
                </div>
                <script>
                    // Función para concatenar tiempo de herramientales
                    function concatenarTiempoHerramientales() {
                        const min = document.getElementById('tiempo_herramientas_min').value;
                        const max = document.getElementById('tiempo_herramientas_max').value;
                        const resultado = min && max ? `${min} a ${max} días hábiles` : '';
                        document.getElementById('tiempo_herramientas').value = resultado;
                    }

                    // Cargar valores iniciales al cargar la página
                    document.addEventListener('DOMContentLoaded', function() {
                        const tiempoActual = '{{ old("tiempo_herramientas", $costeoRequisicion->tiempo_herramientas) }}';
                        if (tiempoActual) {
                            const match = tiempoActual.match(/(\d+)\s*a\s*(\d+)/);
                            if (match) {
                                document.getElementById('tiempo_herramientas_min').value = match[1];
                                document.getElementById('tiempo_herramientas_max').value = match[2];
                            }
                        }
                    });
                </script>
                ...
                <div>
                    ...
                    <div class="flex gap-2 items-center mt-1">
                        <input type="number" id="tiempo_pt_numero" class="rounded-md border-gray-300 shadow-sm p-2 text-center" placeholder="30" oninput="concatenarTiempoPT()">
                        <span class="text-gray-600 font-medium whitespace-nowrap">días hábiles</span>
                    </div>
                    <input type="hidden" name="tiempo_pt" id="tiempo_pt" value="{{ old('tiempo_pt', $costeoRequisicion->tiempo_pt) }}">
                </div>
                <script>
                    // Función para concatenar tiempo PT
                    function concatenarTiempoPT() {
                        const numero = document.getElementById('tiempo_pt_numero').value;
                        const resultado = numero ? `${numero} días hábiles` : '';
                        document.getElementById('tiempo_pt').value = resultado;
                    }

                    // Cargar valor inicial al cargar la página
                    document.addEventListener('DOMContentLoaded', function() {
                        const tiempoPTActual = '{{ old("tiempo_pt", $costeoRequisicion->tiempo_pt) }}';
                        if (tiempoPTActual) {
                            const match = tiempoPTActual.match(/(\d+)/);
                            if (match) {
                                document.getElementById('tiempo_pt_numero').value = match[1];
                            }
                        }
                    });
                </script>
...
<script>
    ...
    function asignarLoteCompraEnResumenPiezas() {
        ...
        document.querySelector('input[name="resumen_piezas_procesos"]').value = 1;
    }
